...
const Settings = () => {
  ...
  const loadProfile = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("profiles")
      .select("full_name, business_name, business_type, plan, plan_end_date")
      .eq("id", user.id)
      .maybeSingle();
    ...
    if (error) {
      toast.error("حصلت مشكلة في تحميل البيانات، جرب تاني");
      return;
    }
    ...
    if (data) {
      ...
      if (data.plan && data.plan !== "free") {
        setPlan({ plan: data.plan, end: data.plan_end_date ?? null });
      } else {
        setPlan(null);
      }
    } else {
      ...
      <section className="rounded-2xl border border-foreground/10 bg-foreground/[0.03] p-5 sm:p-6 space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input value={email} disabled className="w-full rounded-xl border border-foreground/15 bg-foreground/5 px-4 py-3 text-foreground/50" />
          </div>
          <div>
      ...
      <section className="rounded-2xl border border-foreground/10 bg-foreground/[0.03] p-5 sm:p-6">
        ...
        {[
          ...
          <label key={row.k} className="flex items-center justify-between border-b border-foreground/5 py-3 last:border-0">
            ...
            <input
              type="checkbox"
              checked={notif[row.k as keyof typeof notif]}
